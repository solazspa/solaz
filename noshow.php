...
<body>
	...
	<center><h1><b>No Show Schedules</b></h1></center>
	...
	<hr>
						<div class='container'>
							<?php
								...
								echo "<form  method ='get'>
									...
									if(isset($_GET['bookingno']))
									{
										if($_GET['bookingno'] != "")
										{
											$_SESSION['search_booking_no4'] = $_GET['bookingno'];
										}
										else
										{
											unset($_SESSION['search_booking_no4']);
										}
							...
							<div class='form-group'>
										...
										<input type='number' name='bookingno' class='form-control' value ='<?php if(isset($_SESSION['search_booking_no4'])){echo $_SESSION['search_booking_no4'];} ?>' requried>
								</div>
			...
			<table style='padding:10px' class='table table-striped'>
			    ...
			    <tbody>
			    	...
			    	<?php
			    			...
			    			$query = "select * from booking where book_status = 'No Show' order by schedule asc";
								...
								if(isset($_GET['searchdatefrom']) && isset($_GET['searchdateto']))
						    	{
						    		...
						    		if($_GET['searchdatefrom'] != "" && $_GET['searchdateto'] == "")
						    		{
						    			...
						    			$query = "select * from booking where book_status = 'No Show' and schedule like '%".$search_this."%' order by schedule asc";
						    		}
						    		...
						    		if($_GET['searchdatefrom'] != "" && $_GET['searchdateto'] != "")
						    		{
							    			...
							    			$query = "select * from booking where book_status = 'No Show' and (schedule between '".$search_this." 00:00:00' and '".$search_this2." 23:59:59')  order by schedule asc";
						    	...
						    	if(isset($_GET['bookingno']))
						    	{
						    		if(preg_match("/^[0-9]{1,}$/", $_GET['bookingno']))
									{
										...
										$query = "select * from booking where book_status = 'No Show' and bill_id=".$_GET['bookingno']." order by schedule asc";
						    	...
						    	if(isset($_SESSION['search_booking_no4']))
						    	{
						    		$query = "select * from booking where book_status = 'No Show' and bill_id=".$_SESSION['search_booking_no4']." order by schedule asc";
						    	}
						    ...
						    $sess_nameall = "dataNo Show_" . Date('Y') . Date('m') . Date('d') . Date('H') . Date('i') . Date('s');
