...
<body>
	...
	<center><h1><b>Schedule</b></h1></center>
	...
	<hr>
						<div class='container'>
							<?php
								...
								echo "<form  method ='get'>
									...
									if(isset($_GET['bookingno']))
									{
										if($_GET['bookingno'] != "")
										{
											$_SESSION['search_booking_no'] = $_GET['bookingno'];
										}
										else
										{
							...
							<div class='form-group'>
										...
										<input type='number' name='bookingno' class='form-control' value ='<?php if(isset($_SESSION['search_booking_no'])){echo $_SESSION['search_booking_no'];} ?>' requried>
								</div>
			...
			<table style='padding:10px' class='table table-striped'>
			    ...
			    <tbody>
			    	...
			    	<?php
			    			...
			    			$query = "select * from booking where book_status = 'Pending' order by schedule asc";
								...
								if(isset($_GET['searchdatefrom']) && isset($_GET['searchdateto']))
						    	{
						    		...
						    		if($_GET['searchdatefrom'] != "" && $_GET['searchdateto'] == "")
						    		{
						    			...
						    			$query = "select * from booking where book_status = 'Pending' and schedule like '%".$search_this."%' order by schedule asc";
						    		}
						    		...
						    		if($_GET['searchdatefrom'] != "" && $_GET['searchdateto'] != "")
						    		{
							    			...
							    			$query = "select * from booking where book_status = 'Pending' and (schedule between '".$search_this." 00:00:00' and '".$search_this2." 23:59:59')  order by schedule asc";
						    	...
						    	if(isset($_GET['bookingno']))
						    	{
						    		if(preg_match("/^[0-9]{1,}$/", $_GET['bookingno']))
									{
										...
										$query = "select * from booking where book_status = 'Pending' and bill_id=".$_GET['bookingno']." order by schedule asc";
						    	...
						    	if(isset($_SESSION['search_booking_no']))
						    	{
						    		$query = "select * from booking where book_status = 'Pending' and bill_id=".$_SESSION['search_booking_no']." order by schedule asc";
						    	}
							...
							for($x=0;$x<$row2;$x++)
							{
								...
								$noshow = Date('Y-m-d H:i',strtotime($_SESSION[$sess_nameall.'_'.$x.'_4'].' + '.$palugit.' minutes'));
								...
								if($datenow >= $noshow)
								{
									...
									MYSQL_UPDATE("update booking set book_status='No Show' where b_id=".$_SESSION[$sess_nameall.'_'.$x.'_0']);
								...
								else
								{
									...
									<td>".$contact_no."</td>
									<td>".$time."</td>
									<td><a href='pleaseconfirm.php?id=".$_SESSION[$sess_nameall.'_'.$x.'_0']."&status=confirm'>Confirm</a> | <a href='pleaseconfirm.php?id=".$_SESSION[$sess_nameall.'_'.$x.'_0']."&status=cancel'>Cancel</a></td>
