...
<body>
	...
	<center><h1><b>Cancelled Schedules</b></h1></center>
	...
	<hr>
						<div class='container'>
							<?php
								...
								echo "<form  method ='get'>
									...
									if(isset($_GET['bookingno']))
									{
										if($_GET['bookingno'] != "")
										{
											$_SESSION['search_booking_no3'] = $_GET['bookingno'];
										}
										else
										{
											unset($_SESSION['search_booking_no3']);
										}
							...
							<div class='form-group'>
										...
										<input type='number' name='bookingno' class='form-control' value ='<?php if(isset($_SESSION['search_booking_no3'])){echo $_SESSION['search_booking_no3'];} ?>' requried>
								</div>
			...
			<table style='padding:10px' class='table table-striped'>
			    ...
			    <tbody>
			    	...
			    	<?php
			    			...
			    			$query = "select * from booking where book_status = 'Cancelled' order by schedule asc";
								...
								if(isset($_GET['searchdatefrom']) && isset($_GET['searchdateto']))
						    	{
						    		...
						    		if($_GET['searchdatefrom'] != "" && $_GET['searchdateto'] == "")
						    		{
						    			...
						    			$query = "select * from booking where book_status = 'Cancelled' and schedule like '%".$search_this."%' order by schedule asc";
						    		}
						    		...
						    		if($_GET['searchdatefrom'] != "" && $_GET['searchdateto'] != "")
						    		{
							    			...
							    			$query = "select * from booking where book_status = 'Cancelled' and (schedule between '".$search_this." 00:00:00' and '".$search_this2." 23:59:59')  order by schedule asc";
						    	...
						    	if(isset($_GET['bookingno']))
						    	{
						    		if(preg_match("/^[0-9]{1,}$/", $_GET['bookingno']))
									{
										...
										$query = "select * from booking where book_status = 'Cancelled' and bill_id=".$_GET['bookingno']." order by schedule asc";
						    	...
						    	if(isset($_SESSION['search_booking_no3']))
						    	{
						    		$query = "select * from booking where book_status = 'Cancelled' and bill_id=".$_SESSION['search_booking_no3']." order by schedule asc";
						    	}
						    ...
						    $sess_nameall = "dataCancelled_" . Date('Y') . Date('m') . Date('d') . Date('H') . Date('i') . Date('s');
